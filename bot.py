...
def get_youtube_videos(channel_id):
    try:
        ...
        params = {"part": "snippet", "channelId": channel_id, "maxResults": 10, "order": "date", "type": "video", "key": YOUTUBE_API_KEY}
        response = requests.get(url, params=params, timeout=10)
        data = response.json()
        videos = []
...
# ========== ГЛАВНАЯ ПРОВЕРКА (только сегодняшние видео) ==========
def check_all():
    ...
    # Ищем видео, которое вышло СЕГОДНЯ
    today_video = None
    for video in videos:
    ...
    if not today_video:
        send_admin_message(f"📭 Сегодня ({today}) новых видео нет\n\nВремя: {now_str}")
        return
    
    last = load_last_videos()
    last_id = last.get("youtube_id")
    
    logger.info(f"Сегодняшнее видео: {today_video['title']} (ID: {today_video['id']})")
    logger.info(f"Последний отправленный ID: {last_id}")
    
    if last_id != today_video["id"]:
        logger.info("🎬 Отправляем сегодняшнее видео...")
        if send_youtube_video(today_video["thumbnail"], today_video["url"], today_video["title"]):
            last["youtube_id"] = today_video["id"]
            last["last_check"] = now_str
            save_last_videos(last)
            logger.info("✅ Видео отправлено")
    else:
        logger.info("⏭ Сегодняшнее видео уже отправлено")
...
if __name__ == "__main__":
    ...
    logger.info("⏰ Проверка каждый день в 15:00 МСК (только сегодняшние видео)")
